      ...
      <Table sx={{ minWidth: 650 }} aria-label="exposed positions table">
        ...
        <TableBody>
          {exposedPositions.bids.map((bid, index) => (
            <TableRow key={`bid-${index}`}>
              ...
              {exposedPositions.asks[index] ? (
                <TableCell>
                  <CloseIcon sx={{ color: red[500] }} />
                  {exposedPositions.asks[index].type} {exposedPositions.asks[index].amount} @ ${exposedPositions.asks[index].price}
                </TableCell>
              ) : (
                <TableCell></TableCell>
              )}
          ...
          {exposedPositions.asks.length > exposedPositions.bids.length &&
            exposedPositions.asks.slice(exposedPositions.bids.length).map((ask, index) => (
